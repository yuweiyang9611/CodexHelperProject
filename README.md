...
- Vue 3 + TypeScript 仪表盘：以本机用量分析为核心，账户额度为辅助，API 等效金额为参考。Codex 与 Claude Code 分别统计，采用统一查看方式。
...
- 设置管理本机数据源、刷新、通知、价格／历史费率、可选订阅月费对比、托盘与额度悬浮条、更新、备份恢复、索引重建和历史清理。
...
- 金额模块展示当前筛选范围的 API 等效金额，可选对比订阅月费，不称为“回本”或“净收益”。可确认明细按用量发生日期和适用费率重算；未核算金额与零金额明确区分，旧整日估值单独保留，不与重算明细相加。金额不是 API 账单或订阅实际扣费；所有金额使用美元，默认 1,000 点 = US$40。
- 缓存写入按来源可观测字段计价：Claude transcript 的 5 分钟档为基础输入价的 1.25 倍、1 小时档为 2 倍；Codex 本地日志目前不提供 OpenAI 缓存写入拆分，因此这部分仍随普通输入估算，不伪造缓存写入明细。
- 版本化费率目录：设置页可按模型和生效日期维护历史费率版本，记录来源/目录版本，导入、导出或恢复内置目录；历史用量按发生日期重放，未来费率不会回写旧数据。内置目录 `2026.09.2` 已按 2026-09-22 官方发布价补入 GPT-6 Sol／Luna，并保留 GPT-6 Astra、GPT-5.6 Cyber，以及 `gpt-5.6`、Daybreak Blue/Red 官方别名，并按 2026-07-30、2026-08-21 的官方调价日切换 GPT-5.6 Terra/Luna 与 Sol 费率；GPT-5.6 Sol 当前促销价不预设未知的结束价。目录同时保留旧 OpenAI Credits 行与 Anthropic 版本线。GPT Image 2.5 因本地日志缺少文本/图像模态拆分而保持未核算，避免套用错误单价。导入缺少来源与版本标注的旧目录时，按定价特征还原每行自身的出处，而不是统一打上文档级版本。最新来源与价格见 [2026-09-22 费率更新](docs/rate-catalog-2026-09-22.md)。
- 默认仅开启额度不足与额度刷新通知；耗尽预测、金额阈值保留手动开启开关。缺少适用费率默认在金额模块列出模型，未知模型或未归属历史差额不会误报为需要补充费率。
...
Windows 正式发布链已经改为 Electron：直接使用已修复的 Electron Packager 20，生成带限制性 fuses 的 ASAR，携带 Electron/Chromium、项目、Web 与自包含 .NET 运行时许可文件，再依次签名应用与 Sidecar、执行打包态 smoke、构建并安装/运行/卸载测试 Inno Setup，最后生成 ZIP、Setup 和校验和。开机启动失败回滚、窗口工作区/DPI 恢复和 Windows 通知现已迁移；额度状态条已接入，独立桌面仪表盘已退役；Windows 10/11 的混合 DPI 场景仍需在专用环境验收。Linux 适配尚未开始，当前只验证 Windows。
