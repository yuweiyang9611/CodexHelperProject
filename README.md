...
- 用量趋势页支持近 7／30 天的每日堆叠柱状图、按功能／模型切换、分类占比及每日明细；点击图例可突出显示分类。占比以所选期间的本地 Token 为分母，沿用历史留存、工作区筛选和 fork 去重口径。功能目前识别任务与子代理；旧版历史优先从原始日志恢复分类，保留同一运行时、日期和工作区范围内无冲突明细的分类，仅将无法归属的剩余用量保留为未分类。缺少来源标记的明细仍为未分类；不推算官方额度消耗比例，也不伪造自动审查、记忆更新等独立功能用量。
...
- 针对本月等效金额、额度余量和费率覆盖率发送去重通知。费率覆盖提醒仅检查本月已确认模型的用量，并按用量日期匹配当前费率目录；缺少费率时列出具体模型。缺失模型信息、无法归属的旧历史差额和来源冲突不触发补充费率提醒。
