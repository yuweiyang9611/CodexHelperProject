...
| 模型 | 生效日期 | 未缓存输入 / 1M | 缓存输入 / 1M | 输出 / 1M |
| --- | --- | ---: | ---: | ---: |
| GPT-6 Astra | 2026-09-03 | 250 | 25 | 1,250 |
| GPT-6 Sol | 2026-09-22 | 50 | 5 | 250 |
| GPT-6 Luna | 2026-09-22 | 2.5 | 0.25 | 12.5 |
| GPT-5.6 Sol（促销价） | 2026-08-21 | 100 | 10 | 500 |
| GPT-5.6 Terra | 2026-07-30 | 50 | 5 | 300 |
| GPT-5.6 Luna | 2026-07-30 | 5 | 0.5 | 30 |
| GPT-5.6 Cyber | 2026-08-07 | 312.5 | 31.25 | 1,875 |
| Daybreak Blue | 2026-08-21 | 100 | 10 | 500 |
| Daybreak Red | 2026-08-07 | 312.5 | 31.25 | 1,875 |
| GPT-5.5 | 历史基线 | 125 | 12.5 | 750 |
| GPT-5.5 Cyber | 历史基线 | 500 | 50 | 3,000 |
| GPT-5.4 | 历史基线 | 62.5 | 6.25 | 375 |
| GPT-5.4-Mini | 2026-03-17 | 18.75 | 1.875 | 112.5 |
| GPT-5.4-Mini（旧参考表舍入） | 历史基线 | 18.75 | 1.875 | 113 |
| GPT-5.3-Codex | 历史基线 | 43.75 | 4.375 | 350 |
| GPT-5.2 | 历史基线 | 43.75 | 4.375 | 350 |
| GPT-Image-2.0（image） | 历史基线 | 200 | 50 | 750 |
| GPT-Image-2.0（text） | 历史基线 | 125 | 31.25 | 250 |

标注官方来源的 OpenAI 行按 API Standard 短上下文美元价乘以 25 转为点数；GPT-5.4 Mini 从官方发布日采用精确的 112.5 点输出价，旧参考表中的 113 点舍入行仍保留。GPT-5.6 Sol 的当前价格为至少持续至 2026-11-21 的促销价，在官方公布后续价格前不预设回退行。旧版 GPT-5.6 费率仍保存在目录中，并按上述生效日期切换，历史用量不会被新价格回写。Daybreak Blue/Red 因官方说明未来会动态改指向，作为独立的日期化费率历史保存；以后只追加新行，不改写旧请求。

当前 Codex/Claude 本地日志未提供稳定的 API 服务层级，也无法在持久化的按日聚合中重建每次请求是否超过 272K 输入。因此 Batch、Flex、Fast、区域处理和长上下文档位不做猜测，界面展示的是 Standard 短上下文 API 等值估算，而不是 OpenAI API 账单或订阅实际扣费。GPT Image 2.5 同一个模型 ID 同时包含不同的文本、图像输入费率，现有日志没有模态拆分，暂不加入自动核算；无法识别的模型 token 继续计入 `unratedTokens` 并明确提示。

GPT-5.3-Codex-Spark 仍为 research preview，不计入已核算点数。购点参考仅用于刻度：500 点 / US$20、1,000 点 / US$40、2,000 点 / US$80。

费率目录采用 schema 版本与业务版本双层版本化。内置目录当前为 schema `1`、目录版本 `2026.09.2`，新 OpenAI 行来源为官方 API 价目与 Changelog，旧的 `2026.07.1` Credits 参考行继续作为历史基线。每条费率可记录模型、三类点数费率、生效日期、来源、目录版本和匹配方式；生效日期留空表示适用于全部历史。内置目录只允许精确模型/显式别名匹配，避免未来未知后缀被猜价；自定义项默认精确匹配，也可由用户明确选择“前缀家族”。历史用量按发生日期选择当日已经生效的最新费率，自定义目录优先于内置目录，未来生效的版本不得回写历史金额。
